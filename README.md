...
- 데이터베이스 설정은 Generic H2 (Server) 을 사용하였습니다.
...
- H2 DB를 처음 사용하여서 DB 설정에 약간의 어려움을 겪었으나 도큐먼트와 검색을 통한 레퍼런스 참고 하여 해결하였습니다.
- DB 와 연결하는 방법에 대해서 제가 아는 방법인 MyBatis를 사용하였고 @Repository 대신 @Mapper 를 사용하였습니다.
- 배우지 못한 구현 기능에 대해서 (특히 Batch) 구현에 어려움을 겪었으나 역시 도큐먼트와 레퍼런스를 참고하여 해결하였습니다.
- 컨트롤러 Test 에서 제가 아는 Test 방법은 JUnit을 이용하여 DAO나 Service 만 테스트 하는 것이었고 컨트롤러 통합 테스트는 알지 못했으나 레퍼런스를 참고한 끝에 MockMVC 를 이용한 테스트 방법을 알아내서 이를 적용하였습니다.
